...
void	PowerShop::addUI()
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		p_button->addClickEventListener([=](Ref*){
			...
			{
				log("1");
				GAMEDATA->setPowerNum(500); //花一块钱满血
			...
			{
				log("12");
				GAMEDATA->setPowerNum(min(GAMEDATA->getPowerNum()+10,500)); //花12coins钱满血
				float np = (GAMEDATA->getPowerNum() * 100) / 500;
				auto progressTo = ProgressTo::create(1, np);
				progressTimer->runAction(progressTo);
				refreshUI();
			...
			{
				log("20");
				GAMEDATA->setPowerNum(min(GAMEDATA->getPowerNum() + 5, 500)); //花20loves钱满血
				float np = (GAMEDATA->getPowerNum() * 100) / 500;
				auto progressTo = ProgressTo::create(1, np);
				progressTimer->runAction(progressTo);
				refreshUI();
	...
	share_power->addClickEventListener([=](Ref*){
		log("share:1"); //加分享链接
		GAMEDATA->setPowerNum(min(GAMEDATA->getPowerNum()+1,500)); //分享加1 
		float np = (GAMEDATA->getPowerNum() * 100) / 500;
		auto progressTo = ProgressTo::create(1, np);
		progressTimer->runAction(progressTo);
		refreshUI();
